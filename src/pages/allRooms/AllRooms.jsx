...
const AllRooms = () => {
    ...
    const handleLandlords = async () => {
        ...
        try {
            const response = await axios.get(`${backendUrl}homePage/view_properties/room`, {
                headers: {
    ...
    const handleFilter = () => {
        let filtered = [...houses];
        if (selectedTypes.length > 0) {
            filtered = filtered.filter(property => selectedTypes.includes(property.type));
        }
            ...
            <div className="flex flex-col mx-8 h-screen">
                ...
                <div className="flex flex-grow  overflow-hidden">
                    ...
                    <div className="flex-grow p-4 overflow-y-auto no-scrollbar ">
                    {
                            loading ? 
                        <>
                        {(filteredHouses || houses)?.map((house) => (
                            <>
                            <RoomCard key={house._id} property={house} />
                            </>
                        ))}
                        </>
                        :
